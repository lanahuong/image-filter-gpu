...
void run_blur(unsigned int* img, unsigned int* d_img, unsigned int* d_img_tmp, unsigned width, unsigned height, dim3 blockSize, dim3 gridSize) {
...
int main (int argc , char** argv)
{
  ...
  // CPU allocation
  unsigned int *img = (unsigned int*) malloc(alloc_size);
  ...
  // Kernel
  //kernel_sat1<<<gridSize,blockSize>>>(d_img, (unsigned) 2, image_size);
  //kernel_grey<<<gridSize,blockSize>>>(d_img, image_size);
...
/*
  ...
  float kernel[9] = {-1.f, 0.f, 1.f, -2.f, 0.f, 2.f, -1.f, 0.f, 1.f};
  //float kernel[9] = {0.f, 0.2f, 0.f, 0.2f, 0.2f, 0.2f, 0.f, 0.2f, 0.f};
